...
from fivccliche.services.interfaces.configs import (
...
class UserEmbeddingRepositoryImpl(UserEmbeddingRepository):
    ...
    def __init__(self, user_id: str | None = None, session: AsyncSession | None = None):
    ...
    async def update_embedding_config_async(self, embedding_config: EmbeddingConfig) -> None:
        ...
        if not self.session or not self.user_id:
            raise ValueError(
                "Session and user_id are required for update_embedding_config operation"
            )
        # Check if config exists
        existing = await methods.get_embedding_config_async(
            self.session, embedding_config.id, self.user_id
        )
        if existing:
            ...
            await methods.create_embedding_config_async(
                self.session, self.user_id, embedding_config
            )

    async def get_embedding_config_async(self, embedding_id: str) -> EmbeddingConfig | None:
        """Retrieve an embedding configuration by ID."""
        if not self.session or not self.user_id:
            raise ValueError("Session and user_id are required for get_embedding_config operation")
        config = await methods.get_embedding_config_async(self.session, embedding_id, self.user_id)
    ...
    async def list_embedding_configs_async(self, **kwargs) -> list[EmbeddingConfig]:
        ...
        if not self.session or not self.user_id:
            raise ValueError(
                "Session and user_id are required for list_embedding_configs operation"
            )
        skip = kwargs.get("skip", 0)
        limit = kwargs.get("limit", 100)
        configs = await methods.list_embedding_configs_async(
            self.session, self.user_id, skip=skip, limit=limit
        )
    ...
    async def delete_embedding_config_async(self, embedding_id: str) -> None:
        ...
        if not self.session or not self.user_id:
            raise ValueError(
                "Session and user_id are required for delete_embedding_config operation"
            )
        config = await methods.get_embedding_config_async(self.session, embedding_id, self.user_id)
        if config:
...
class UserLLMRepositoryImpl(UserLLMRepository):
    ...
    async def update_model_config_async(self, model_config: ModelConfig) -> None:
        """Create or update a model configuration."""
        if not self.session or not self.user_id:
            raise ValueError("Session and user_id are required for update_model_config operation")
        # Check if config exists
        existing = await methods.get_llm_config_async(self.session, model_config.id, self.user_id)
        if existing:
            # Update existing config
            await methods.update_llm_config_async(self.session, existing, model_config)
        else:
            # Create new config
            await methods.create_llm_config_async(self.session, self.user_id, model_config)

    async def get_model_config_async(self, model_id: str) -> ModelConfig | None:
        """Retrieve a model configuration by ID."""
        if not self.session or not self.user_id:
            raise ValueError("Session and user_id are required for get_model_config operation")
        config = await methods.get_llm_config_async(self.session, model_id, self.user_id)
        return config.to_config() if config else None

    async def list_model_configs_async(self, **kwargs) -> list[ModelConfig]:
        """List all model configurations in the repository."""
        if not self.session or not self.user_id:
            raise ValueError("Session and user_id are required for list_model_configs operation")
        skip = kwargs.get("skip", 0)
        limit = kwargs.get("limit", 100)
    ...
    async def delete_model_config_async(self, model_id: str) -> None:
        """Delete a model configuration."""
        if not self.session or not self.user_id:
            raise ValueError("Session and user_id are required for delete_model_config operation")
        config = await methods.get_llm_config_async(self.session, model_id, self.user_id)
        if config:
...
class UserAgentRepositoryImpl(UserAgentRepository):
    ...
    async def update_agent_config_async(self, agent_config: AgentConfig) -> None:
        """Create or update an agent configuration."""
        if not self.session or not self.user_id:
            raise ValueError("Session and user_id are required for update_agent_config operation")
        # Check if config exists
        existing = await methods.get_agent_config_async(self.session, agent_config.id, self.user_id)
        if existing:
            # Update existing config
            await methods.update_agent_config_async(self.session, existing, agent_config)
        else:
            # Create new config
            await methods.create_agent_config_async(self.session, self.user_id, agent_config)

    async def get_agent_config_async(self, agent_id: str) -> AgentConfig | None:
        """Retrieve an agent configuration by ID."""
        if not self.session or not self.user_id:
            raise ValueError("Session and user_id are required for get_agent_config operation")
        config = await methods.get_agent_config_async(self.session, agent_id, self.user_id)
        return config.to_config() if config else None

    async def list_agent_configs_async(self) -> list[AgentConfig]:
        """List all agent configurations in the repository."""
        if not self.session or not self.user_id:
            raise ValueError("Session and user_id are required for list_agent_configs operation")
        configs = await methods.list_agent_configs_async(self.session, self.user_id)
        return [config.to_config() for config in configs]

    async def delete_agent_config_async(self, agent_id: str) -> None:
        """Delete an agent configuration."""
        if not self.session or not self.user_id:
            raise ValueError("Session and user_id are required for delete_agent_config operation")
        config = await methods.get_agent_config_async(self.session, agent_id, self.user_id)
        if config:
...
class ConfigProviderImpl(IUserConfigProvider):
    ...
    def get_embedding_repository(
        ...
        **kwargs,  # ignore additional arguments
        ...
        return UserEmbeddingRepositoryImpl(user_id=user_id, session=session)
    ...
    def get_model_repository(
        ...
        **kwargs,  # ignore additional arguments
        ...
        return UserLLMRepositoryImpl(user_id=user_id, session=session)
    ...
    def get_agent_repository(
        ...
        **kwargs,  # ignore additional arguments
        ...
        return UserAgentRepositoryImpl(user_id=user_id, session=session)
