...
class UserASR(SQLModel, table=True):
    """ASR model configuration."""

    __tablename__ = "user_asr"
    __table_args__ = (Index("ix_user_asr_id_user_uuid", "id", "user_uuid", unique=True),)

    uuid: str = Field(
        default_factory=lambda: str(uuid4()),
        primary_key=True,
        max_length=36,
        description="ASR config UUID.",
    )
    id: str = Field(
        max_length=36,
        description="ASR config ID (unique within user scope).",
        index=True,
    )
    description: str | None = Field(default=None, max_length=1024, description="ASR description.")
    model: str = Field(
        max_length=255,
    ...
    def to_schema(self) -> schemas.UserASRSchema:
        return schemas.UserASRSchema(
            uuid=self.uuid,
            id=self.id,
            description=self.description,
            model=self.model,
